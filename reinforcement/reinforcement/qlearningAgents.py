...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        for action in self.getLegalActions(state):
            ...
            maxvalue = max(maxvalue, currqvalue)
        return maxvalue
    ...
    def computeActionFromQValues(self, state):
        ...
        if len(self.getLegalActions(state)) == 0:
            ...
            for actions in self.getLegalActions(state):
                currValue = self.getQValue(state, actions)
                if currValue > max:
                    decision = actions
                    max = currValue
        return decision
    ...
    def update(self, state, action, nextState, reward):
        ...
        self.value[(state, action)] =  self.alpha * val+(1 - self.alpha) * self.getQValue(state, action)
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        for feature in self.featExtractor.getFeatures(state, action):
            self.weights[feature] = (self.alpha * different * self.featExtractor.getFeatures(state, action)[feature])+self.weights[feature]

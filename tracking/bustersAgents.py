...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        nearestGhost = None
        GhostDist = float("Inf")
        for ghostnum, pd in enumerate(livingGhostPositionDistributions):
            pos = pd.argMax()
        ...
        return getfinalaction(self,nearestGhost,pacmanPosition,legal)

def getfinalaction(self,nearestGhost,pacmanPosition,legal):

        minDist = float("Inf")
        Action = None
        for action in legal:
            sPosition = Actions.getSuccessor(pacmanPosition, action)
            newDist = self.distancer.getDistance(sPosition, nearestGhost)
            if newDist < minDist:
                minDist = newDist
                Action = action
